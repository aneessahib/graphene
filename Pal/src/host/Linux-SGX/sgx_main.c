...
static int load_enclave(struct pal_enclave* enclave, int manifest_fd, char* manifest_uri,
    ...
    if (enclave->manifest_loader) {
        toml_raw_t loader_exec_raw = toml_raw_in(enclave->manifest_loader, "exec");
        if (loader_exec_raw) {
            char* loader_exec = NULL;
            ret = toml_rtos(loader_exec_raw, &loader_exec);
            if (ret < 0) {
                SGX_DBG(DBG_E, "Cannot read \'loader.exec\' (it must be put in quotes!)\n");
                return -EINVAL;
            }
    ...
    if (!sgx_sigfile_raw) {
        SGX_DBG(DBG_E, "Sigstruct not found (\'sgx.sigfile\' must be specified in manifest)\n");
        return -EINVAL;
    }

    char* sgx_sigfile = NULL;
    ret = toml_rtos(sgx_sigfile_raw, &sgx_sigfile);
    if (ret < 0) {
        SGX_DBG(DBG_E, "Cannot read \'sgx.sigfile\' (it must be put in quotes!)\n");
        return -EINVAL;
    }

    char* sig_uri = resolve_uri(sgx_sigfile, &errstring);
    if (!sig_uri) {
    ...
    if (!strendswith(sig_uri, ".sig")) {
        SGX_DBG(DBG_E, "Invalid sigstruct file (must have extension \'.sig\'): %s\n", sig_uri);
        free(sig_uri);
        return -EINVAL;
    }
