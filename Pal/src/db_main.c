...
noreturn void pal_main(PAL_NUM instance_id,        /* current instance id */
    ...
    if (!exec_handle && g_pal_state.manifest_loader) {
        toml_raw_t loader_exec_raw = toml_raw_in(g_pal_state.manifest_loader, "exec");
        if (loader_exec_raw) {
            ret = toml_rtos(loader_exec_raw, &exec_uri);
            if (ret < 0)
                INIT_FAIL_MANIFEST(PAL_ERROR_DENIED, "Cannot read \'loader.exec\'");
